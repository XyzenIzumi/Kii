...
async function imageToWebp (media) {
    const tmpFileOut = path.join(os.tmpdir(), `${getRandom(1)}.webp`)
    const tmpFileIn = path.join(os.tmpdir(), `${getRandom(1)}.jpg`)
...
async function videoToWebp (media) {
    const tmpFileOut = path.join(os.tmpdir(), `${getRandom(1)}.webp`)
    const tmpFileIn = path.join(os.tmpdir(), `${getRandom(1)}.mp4`)
    fs.writeFileSync(tmpFileIn, media)
    await new Promise((resolve, reject) => {
        ffmpeg(tmpFileIn)
            .on("error", reject)
...
async function writeExifImg (media, metadata) {
    ...
    const tmpFileIn = path.join(os.tmpdir(), `${getRandom(1)}.webp`)
    const tmpFileOut = path.join(os.tmpdir(), `${getRandom(1)}.webp`)
